    ...
    render(){
        ...
        return(
            <div className={`product-card ${inStock?true:"out-stock"}`}>
                <div className="layer" style={{display:inStock?"none":"flex"}}><h1>OUT OF STOCK</h1></div>
                <Link to={`/product/${id}`}><img src={gallery[0]} className="product-image" alt ={id} /></Link>
                <div className={`details-section ${inStock?"details-section-in-stock":""}` }>
                    <button className="add-to-cart" style={{display:inStock?"":"none"}} onClick={()=>{this.props.addToCart({unique:this.generateKey(),id,name,prices,gallery,brand,attributes,selectedAttrs:defaultAttributes, count:1})}}>
